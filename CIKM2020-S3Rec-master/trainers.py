...
from utils import recall_at_k, ndcg_k, get_metric, FairnessMetrics
...
class Trainer:
    def __init__(self, model, train_dataloader,
        ...
        self._setup_fairness_metrics()
        
    def _setup_fairness_metrics(self):
        item_to_category = getattr(self.args, 'item_to_category', {})
        user_category_profiles = getattr(self.args, 'user_category_profiles', {})
        item_popularity = getattr(self.args, 'item_popularity', {})
        user_popularity_profiles = getattr(self.args, 'user_popularity_profiles', {})  # NEW
        # total_items = getattr(self.args, 'item_size', None)
        total_items = getattr(self.args, 'actual_total_items', getattr(self.args, 'item_size', None))

        self.fairness_metrics = FairnessMetrics(
            item_to_category=item_to_category,
            user_category_profiles=user_category_profiles,
            item_popularity=item_popularity,
            user_popularity_profiles=user_popularity_profiles,  # NEW
    ...
    def get_full_sort_score(self, epoch, answers, pred_list, user_ids=None):
        recall, ndcg = [], []
        ...
        post_fix = {
            "Epoch": epoch,
            "HIT@1": '{:.4f}'.format(recall[0]), "NDCG@1": '{:.4f}'.format(ndcg[0]),
            "HIT@5": '{:.4f}'.format(recall[1]), "NDCG@5": '{:.4f}'.format(ndcg[1]),
            "HIT@10": '{:.4f}'.format(recall[2]), "NDCG@10": '{:.4f}'.format(ndcg[2]),
            "HIT@20": '{:.4f}'.format(recall[4]), "NDCG@20": '{:.4f}'.format(ndcg[4])
        }
        
        # computing fairness metrics  here in case of have user_ids
        if user_ids is not None and self.fairness_metrics is not None:
            try:
                fairness_results = self.fairness_metrics.compute_metrics_for_predictions(
                    user_ids, pred_list, ks=[1, 5, 10]
                )

                # global coverage fix 
                from collections import defaultdict
                global_predictions = defaultdict(set)
                for i in range(len(pred_list)):
                    for k in [1, 5, 10]:
                        topk_items = pred_list[i][:k]
                        for item in topk_items:
                            global_predictions[k].add(item)
                
                for k in [1, 5, 10]:
                    unique_items = len(global_predictions[k])
                    coverage = unique_items / self.fairness_metrics.total_items
                    fairness_results[f"coverage@{k}"] = coverage
                    print(f"GLOBAL Coverage@{k}: {unique_items}/{self.fairness_metrics.total_items} = {coverage:.4f}")

                for key, value in fairness_results.items():
                    post_fix[key] = '{:.4f}'.format(value)
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    if i == 0:
                        ...
                        user_id_list = user_ids.cpu().data.numpy() #NEW
                    else:
                        pred_list = np.append(pred_list, batch_pred_list, axis=0)
                        answer_list = np.append(answer_list, answers.cpu().data.numpy(), axis=0)
                        user_id_list = np.append(user_id_list, user_ids.cpu().data.numpy(), axis=0) #NEW
                return self.get_full_sort_score(epoch, answer_list, pred_list, user_id_list) # NEW
