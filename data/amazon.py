...
class AmazonReviews23(InMemoryDataset, PreprocessingMixin):
    ...
    def train_test_split(self, pdf_reviews, data_maps, max_seq_len=20):
        ...
        for uid, grp in pdf_reviews.sort_values("timestamp").groupby("user_id", sort=False):
            ...
            user_sequences.append((uid, item_ids))
        ...
        for uid, item_ids in user_sequences:

            train_items = item_ids[:-2]
            eval_items = item_ids[-(max_seq_len + 2):-2]
            test_items = item_ids[-(max_seq_len + 1):-1]
            ...
            if self.strong_generalization:
                ...
                sequences[split]["itemId"].append(eval_padded if split == "eval" else test_padded if split == "test" else train_items)
                sequences[split]["itemId_fut"].append(item_ids[-1] if split == "test" else item_ids[-2])
                sequences[split]["userId"].append(data_maps["user2id"][uid])
            else:
                sequences["train"]["itemId"].append(train_items)
                sequences["train"]["itemId_fut"].append(item_ids[-2])
                sequences["train"]["userId"].append(data_maps["user2id"][uid])

                sequences["eval"]["itemId"].append(eval_padded)
                sequences["eval"]["itemId_fut"].append(item_ids[-2])
                sequences["eval"]["userId"].append(data_maps["user2id"][uid])

                sequences["test"]["itemId"].append(test_padded)
                sequences["test"]["itemId_fut"].append(item_ids[-1])
                sequences["test"]["userId"].append(data_maps["user2id"][uid])
    ...
    def process(self, max_seq_len=20) -> None:
        ...
        sequences = self.train_test_split(pdf_reviews, data_maps, max_seq_len=max_seq_len)
        data["user","rated","item"].history = {
        ...
        # Create brand mapping
        unique_brands = item_data[self.category].unique().tolist()
        self.brand_mapping = {i: b for i, b in enumerate(unique_brands)}

        # Create reverse mapping for lookup
        brand_to_id = {b: i for i, b in self.brand_mapping.items()}

        # Add brand_id to item_data
        item_data["brand_id"] = item_data[self.category].map(lambda x: brand_to_id.get(x, -1))

        sentences = (
            "Title: " + item_data["title"].astype(str) + "; " +
            "Brand: " + item_data[self.category].astype(str) + "; " +
        ...
        data["item"].brand_id = torch.tensor(item_data["brand_id"].values)
