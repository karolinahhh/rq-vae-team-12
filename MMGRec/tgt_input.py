...
class GCN(torch.nn.Module):
    def __init__(self, user_num, item_num, feat_num):
        super().__init__()
        self.conv1 = SAGEConv(128, 128)
        self.conv2 = SAGEConv(128, 64)
        self.USER_NUM = user_num
        self.ITEM_NUM = item_num
        self.fuse = nn.Linear(feat_num, 128, bias=False)
...
v_f_lookup = np.load('MMGRec/dataset_sample/v_feat_sample.npy', allow_pickle=True)  # size=(item_num, 128)
a_f_lookup = np.load('MMGRec/dataset_sample/a_feat_sample.npy', allow_pickle=True)
t_f_lookup = np.load('MMGRec/dataset_sample/t_feat_sample.npy', allow_pickle=True)
f_lookup = torch.from_numpy(np.concatenate((v_f_lookup, a_f_lookup, t_f_lookup), axis=1)).float().to(device)

feat_dim = f_lookup.shape[1]
...
for b_i in list(range(data_block)):
    triple_para = pickle.load(open('results/triple_' + str(b_i) + '.para', 'rb'))
...
gcn = GCN(user_num, item_num, feat_dim).to(device)
...
for epoch in range(epoch_max):
    ...
    for step, (batch_i, batch_j, batch_m) in enumerate(train_loader):
        ...
        embedding_i = out[batch_i]                   # out is already on device
        embedding_j = out[batch_j + user_num]
        embedding_m = out[batch_m + user_num]
...
with torch.no_grad():
    ...
    out = out.cpu().numpy()
    np.save('results/representation.npy', out)
...
l_w_embedding = 1
l_w_commitment = 0.25
content = np.load('results/representation.npy')[user_num: user_num + item_num, :]
...
col = np.zeros(r_id.shape[0], dtype=np.int8)
...
np.save('results/tgt_mtx.npy', tgt_mtx)
print('finish!')
