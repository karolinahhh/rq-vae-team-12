...
whole_dict = np.load('MMGRec/dataset_sample/user_item_dict_sample.npy', allow_pickle=True)
whole_dict = whole_dict.item()

USER_NUM = len(whole_dict)
all_items = set()
for items in whole_dict.values():
    all_items.update(items)
ITEM_NUM = max(all_items) + 1
...
for k in range(USER_NUM):
    ...
    test_set = random.sample(test_val_set, math.ceil(0.5 * len(test_val_set)))
...
pickle.dump(para, open('results/load.para', 'wb'))
