...
src_mtx = np.zeros((user_num, src_len), dtype=np.int8)  # each row is a user token
...
for u, row in enumerate(train_matrix):
    ...
    if len(itr) >= src_len:
        ...
        # itr = np.sort(itr)
        u_src = np.append(itr, np.zeros(src_len - len(itr), dtype=np.int8))
    src_mtx[u] = u_src

np.save('results/src_mtx.npy', src_mtx)
print('finish!')